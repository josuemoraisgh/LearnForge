# -*- coding: utf-8 -*-
"""
Janela principal da aplicação (Tk + ttk).
Entrada de múltiplos JSONs via tabela (uma coluna).
Ajustes:
- Altura visual máxima de 5 linhas (sem scroll visível quando <=5 itens; aparece quando >5).
- "Revisar/Editar Questões…" robusto: tenta import absoluto e tem fallback por caminho do arquivo
  para evitar "attempted relative import beyond top-level package".
"""
import io
import json
import shutil
...
from config.preferences import APP_NAME, INI_PATH, FONT_SIZES, DEFAULTS, load_prefs, save_prefs
...
class App(ttk.Frame):
    ...
    def _build_top(self):
        ...
        # Scrollbar: só mostra quando >5 itens
        self._update_scrollbar_visibility()

        files.rowconfigure(0, weight=0)  # altura fixa visual (sem expandir)
    ...
    def _get_question_editor_class(self):
        # Tentativa 1: import absoluto (quando rodando como pacote)
        try:
            from json2beamer_app.editor.question_editor import QuestionEditor  # type: ignore
            return QuestionEditor
        except Exception:
            pass
        # Tentativa 2: import relativo padrão (se pacote estiver correto)
        try:
            from ..editor.question_editor import QuestionEditor  # type: ignore
            return QuestionEditor
        except Exception:
            pass
        # Tentativa 3: carregamento por caminho de arquivo (quando rodando "flat")
        here = Path(__file__).resolve()
        editor_path = (here.parent.parent / "editor" / "question_editor.py").resolve()
